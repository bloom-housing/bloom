...
export class FileServiceProvider {
  ...
  configure(config: FileServiceConfig, varPrefix = '') {
    if (!this._registeredServices[this._activeServiceName]) {
      throw new Error(
        `Cannot configure unregistered file service "${this._activeServiceName}"`,
      );
    }
    ...
    this._activeService = this._initService(
      this._activeServiceName,
      activeConfig,
    );
    return this;
  }
  ...
  private _initService(name: string, config: FileServiceConfig) {
    ...
    if (!service) {
      throw new Error(
        `Cannot load unregistered file service [${this._activeServiceName}]`,
      );
    }
