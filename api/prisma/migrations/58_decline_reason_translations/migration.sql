...
WITH updated AS (
  UPDATE translations
  SET translations = jsonb_set(jsonb_set(COALESCE(translations, '{}'::jsonb), '{applicationUpdate,declineReasonChange}', '"您的申请被拒原因是 %{value}"'::jsonb, true), '{applicationUpdate,declineReason}', '{"ageRestriction":"年龄限制","incomeRestriction":"收入限制","unitRestriction":"单元限制（例如卧室数量和/或入住人数）","programRestriction":"项目限制（例如 CES 单位、退伍军人单位、VASH 单位等）","attemptedToContactNoResponse":"尝试联系，但未收到回复","householdDoesNotNeedAccessibleUnit":"家庭不需要无障碍单元功能","other":"其他"}'::jsonb, true)
...
WITH updated AS (
  UPDATE translations
  SET translations = jsonb_set(jsonb_set(COALESCE(translations, '{}'::jsonb), '{applicationUpdate,declineReasonChange}', '"귀하의 지원서가 거절된 이유는 %{value}입니다."'::jsonb, true), '{applicationUpdate,declineReason}', '{"ageRestriction":"연령 제한","incomeRestriction":"소득 제한","unitRestriction":"숙소 제한 사항 (예: 침실 개수 및/또는 거주 인원)","programRestriction":"프로그램 제한 사항 (예: CES 부대, 재향군인 부대, VASH 부대 등)","attemptedToContactNoResponse":"연락을 시도했으나 응답이 없었습니다.","householdDoesNotNeedAccessibleUnit":"해당 가구는 장애인 편의시설이 필요한 주택이 아닙니다.","other":"다른"}'::jsonb, true)
...
WITH updated AS (
  UPDATE translations
  SET translations = jsonb_set(jsonb_set(COALESCE(translations, '{}'::jsonb), '{applicationUpdate,declineReasonChange}', '"دلیل رد درخواست شما %{value} است."'::jsonb, true), '{applicationUpdate,declineReason}', '{"ageRestriction":"محدودیت سنی","incomeRestriction":"محدودیت درآمد","unitRestriction":"محدودیت واحد (مثلاً تعداد اتاق خواب و/یا میزان اشغال)","programRestriction":"محدودیت برنامه (مثلاً واحد CES، واحد کهنه سربازان، واحد VASH و غیره)","attemptedToContactNoResponse":"تلاش برای تماس؛ بدون پاسخ","householdDoesNotNeedAccessibleUnit":"خانوار به ویژگی‌های واحد قابل دسترس نیاز ندارد","other":"دیگر"}'::jsonb, true)
