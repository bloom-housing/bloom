...
describe("useTranslationScope", () => {
  ...
  it("offers the jurisdiction's languages in the public scope", () => {
    ...
    expect(result.current.languageOptions.map((option) => option.value)).toEqual(["en", "es"])
  })
  ...
  it("does not ask for the English overrides while editing English", async () => {
    const { result } = renderScope([jurisdiction("first")])

    await waitFor(() => expect(result.current.overrides).toEqual([]))
    // One read for the scope itself, and no second read for a comparison against itself.
    expect(translationsService.getRawTranslations).toHaveBeenCalledTimes(1)
