describe("Listing Management Tests", () => {
  ...
  function fillOutListing(cy: Cypress.cy, listing: any): void {
    ...
    cy.getByID("addUnitsButton").should("exist").contains("Add unit").click()
    cy.getByID("number").should("exist").type(listing["number"])
    cy.getByID("unitTypes.id").should("exist").select(listing["unitType.id"])
    cy.getByID("numBathrooms").should("exist").select(listing["numBathrooms"])
    cy.getByID("floor").should("exist").select(listing["floor"])
    cy.getByID("sqFeet").should("exist").type(listing["sqFeet"])
    cy.getByID("minOccupancy").should("exist").select(listing["minOccupancy"])
    cy.getByID("maxOccupancy").should("exist").select(listing["maxOccupancy"])
    cy.getByID("fixed").should("exist").check({ force: true })
    cy.getByID("monthlyIncomeMin").should("exist").type(listing["monthlyIncomeMin"])
    cy.getByID("monthlyRent").should("exist").type(listing["monthlyRent"])
    cy.getByID("unitAccessibilityPriorityTypes.id")
      .should("exist")
      .select(listing["priorityType.id"])
    cy.get("button").should("exist").contains("Save & exit").click()
    cy.getByID("amiChart.id").should("exist").select(1).trigger("change")
    cy.getByID("amiPercentage").should("exist").select(1)
    cy.get("button").should("exist").contains("Save & exit").click()
