...
import { getCurrencyRange, tIfExists } from "@bloom-housing/shared-helpers"
    ...
    <>
      ...
      creditScreeningFee ? (
        <>
                ...
                {(depositMin || depositMax || depositHelperText || depositValue) && (
                  <div className={styles["split-card-cell"]}>
                    ...
                    {isNonRegulated &&
                      ...
                      depositValue && (
                        <div className={styles.emphasized}>{`$ ${depositValue}`}</div>
                      )}
          ...
          {tIfExists("listings.additionalFeesDisclaimer") && (
            <p className={`seeds-p-bs-4`}>{t("listings.additionalFeesDisclaimer")}</p>
          )}
        </>
      ) : null}
    </>
