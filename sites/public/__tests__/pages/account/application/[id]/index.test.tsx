...
import { ApplicationsService, ListingsService } from '@bloom-housing/shared-helpers/src/types/backend-swagger'
...
describe("Account Listing View", () => {
  ...
  it("should render all application details", async () => {
    ...
    // Listing heading
    expect(await screen.findByRole('heading', { level: 1, name: /archer studios/i })).toBeInTheDocument()
    ...
    expect(backButton).toBeInTheDocument()
    expect(backButton).toHaveAttribute("href", "/account/applications")
    expect(screen.getByRole("heading", { level: 2, name: /here's the information you submitted./i })).toBeInTheDocument()
    ...
    expect(within(applicantNameSection).getByText(/Applicant First Applicant Middle Applicant Last/i)).toBeInTheDocument();
    ...
    const applicantAdditionalPhoneSection = screen.getByTestId('app-summary-applicant-additional-phone');
    expect(applicantAdditionalPhoneSection).toBeInTheDocument();
    expect(within(applicantAdditionalPhoneSection).getByText(/additional phone/i)).toBeInTheDocument();
    expect(within(applicantAdditionalPhoneSection).getByText(/\(456\) 456-4564/i)).toBeInTheDocument();
    ...
    expect(within(applicantAddressSection).getByText(/3200 Old Faithful Inn Rd/i)).toBeInTheDocument();

    const applicantMailingAddressSection = screen.getByTestId('app-summary-applicant-mailing-address');
    ...
    expect(within(applicantMailingAddressSection).getByText(/estes park, CO 80517/i)).toBeInTheDocument();
    ...
    expect(within(applicantWorkAddressSection).getByText(/yosemite valley, CA 95389/i)).toBeInTheDocument();
    ...
    expect(within(contactPreferenceSection).getByText(/preferred contact type/i)).toBeInTheDocument();
    expect(within(contactPreferenceSection).getByText(/email/i)).toBeInTheDocument();

    // --------------------------- Alternate Contact ------------------------------------

    expect(screen.getByRole('heading', { level: 3, name: /alternate contact/i })).toBeInTheDocument()
    ...
    expect(within(alternateNameSection).getByText(/Alternate First Alternate Last/i)).toBeInTheDocument();
    ...
    const alternateMailingAddressSection = screen.getByTestId('app-summary-alternate-mailing-address');
    ...
    expect(within(alternateMailingAddressSection).getByText(/acadia national park/i)).toBeInTheDocument();
    expect(within(alternateMailingAddressSection).getByText(/25 visitor center rd /i)).toBeInTheDocument();
    expect(within(alternateMailingAddressSection).getByText(/bay harbor, me 04609/i)).toBeInTheDocument();
    ...
    expect(within(adaAccessibleUnitsSection).getByTestId('For Vision Impairments')).toBeInTheDocument();
    expect(within(adaAccessibleUnitsSection).getByTestId('For Hearing Impairments')).toBeInTheDocument();
    expect(within(adaAccessibleUnitsSection).getByTestId('For Mobility Impairments')).toBeInTheDocument();
    ...
    expect(within(householdChangesSection).getByText(/expecting household changes/i)).toBeInTheDocument();
    ...
    expect(within(householdStudentSection).getByText(/household includes student or member nearing 18/i)).toBeInTheDocument();
    ...
    expect(within(vouchersSection).getByText(/housing voucher or rental subsidy/i)).toBeInTheDocument()
    ...
    expect(within(workInCitySection1).getByText(/at least one member of my household works in the city/i)).toBeInTheDocument();
    ...
    expect(within(workInCitySection2).getByText(/all members of the household work in the city/i)).toBeInTheDocument();
    ...
    expect(within(cityEmployeesSection).getByText(/at least one member of my household is a city employee/i)).toBeInTheDocument();

    // Card Footer
    expect(screen.getByRole('button', { name: /print a copy for your records/i })).toBeInTheDocument()
  })
  ...
  it("should run windo print on button click", async () => {
    ...
    const printButton = await screen.findByRole('button', { name: /print a copy for your records/i })
