...
describe("Listings map", function () {
  it("renders the listing map", function () {
    ...
    cy.intercept("**/listings/list", (req) => {
      req.headers["language"] = "en"
    }).as("listingsSearch")
    ...
    // Initial map load
    cy.getByTestId("map-total-results").contains("Total results 243")

    cy.getByTestId("map-pagination").contains("Page 1 of 10")
    cy.getByTestId("map-cluster").should("have.length", 10)
    cy.getByTestId("loading-overlay").should("not.exist")
    ...
    cy.getByID("monthlyRent.minRent").type("1")
    cy.getByID("monthlyRent.maxRent").type("1")
    cy.getByID("show-listings-button").click()
    ...
    // Clear filters
    cy.getByID("listings-map-filter-button").contains("Filters 2").click()
    cy.getByID("clear-listings-button").click()
    cy.getByTestId("loading-overlay").should("not.exist")
    ...
    cy.getByTestId("map-cluster").should("have.length", 20)
