...
output "ses_details" {
  value = [
    for i in toset(var.ses_identities) : {
      identity                    = i
      verification_status         = aws_sesv2_email_identity.domains[i].verification_status
      verified_for_sending_status = aws_sesv2_email_identity.domains[i].verified_for_sending_status
      ...
      validation_dns_records = strcontains(i, "@") ? null : [for t in aws_sesv2_email_identity.domains[i].dkim_signing_attributes[0].tokens : {
        record_type  = "CNAME"
