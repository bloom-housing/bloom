...
We are using [gitleaks-action](https://github.com/gitleaks/gitleaks-action) to reject pushing up secrets and api keys. Optionally you can enable the pre-commit hook to check for these patterns on commit instead. The `.githooks` directory contains a pre-commit script to check for secrets before allowing a `git commit`. To enable this for your local clone please perform the following steps:
