...
export class detroitEmailUpdates1649709708991 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const translation = await queryRunner.query(
      `SELECT id, translations FROM translations WHERE language='en' AND jurisdiction_id = '${jurisdiction}'`
    )
    ...
    translations.register.welcomeMessage =
      "Thank you for setting up your account on %{appUrl}. It will now be easier to save listings that you are interested in on the site."
