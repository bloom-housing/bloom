...
import unit from "../../test/factories/unit"
...
export class refactorUnitTypesRentTypesAndPriorityTypes1625825502154 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // await queryRunner.query(`CREATE TABLE "unit_types" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "name" text NOT NULL, CONSTRAINT "PK_105c42fcf447c1da21fd20bcb85" PRIMARY KEY ("id"))`);
        //
        // await queryRunner.query(`ALTER TABLE "units" ADD "unit_type_id" uuid`);
        // await queryRunner.query(`ALTER TABLE "units" ADD CONSTRAINT "FK_1e193f5ffdda908517e47d4e021" FOREIGN KEY ("unit_type_id") REFERENCES "unit_types"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        // for (const unitType of ["studio", "oneBdrm", "twoBdrm", "threeBdrm", "fourBdrm"]) {
        //     const newUnitType = await queryRunner.query(`INSERT INTO "unit_types" (name) VALUES ($1)`, [unitType])
        //     const unitsToBeUpdated = await queryRunner.query(`SELECT id, unit_type FROM units where unit_type = ($1)`, [unitType])
        //     for(const unit of unitsToBeUpdated) {
        //         await queryRunner.query(`UPDATE units SET unit_type_id = ($1) WHERE id = ($2)`, [newUnitType.id, unit.id])
        //     }
        // }
        //
        ...
        // await queryRunner.query(`CREATE TABLE "unit_accessibility_priority_types" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "name" text NOT NULL, CONSTRAINT "PK_2cf31d2ceea36e6a6b970608565" PRIMARY KEY ("id"))`);
