...
export class newStatusFields1691377869351 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."listings_status_enum" AS ENUM('active', 'changesRequested', 'closed', 'pending', 'pendingReview')`
    )
    await queryRunner.query(`ALTER TABLE "listings" ALTER COLUMN "status" DROP DEFAULT`)
    await queryRunner.query(
      `ALTER TABLE "listings" ALTER COLUMN "status" TYPE "public"."listings_status_enum" USING "status"::"text"::"public"."listings_status_enum"`
    )
