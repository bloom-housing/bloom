...
export class addApplicationMethodBooleans1630777068604 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "listings" ADD "common_digital" boolean NOT NULL DEFAULT true`
    )
    await queryRunner.query(
    ...
    const applicationMethods = await queryRunner.query(
      `SELECT type, listing_id FROM application_methods WHERE type IN ('ExternalLink', 'FileDownload')`
    )
