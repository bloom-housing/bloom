...
export class addPublicUrlToJurisdiction1639561971201 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const jurisdictions: Array<{id: string, name: string, public_url: string}> = await queryRunner.query(`SELECT id, name, public_url from jurisdictions`);
        ...
        for (const jurisdiction of [alamedaJurisdiction, sanJoseJurisdiction, sanMateoJurisdiction]) {
            await queryRunner.query(`UPDATE jurisdictions SET public_url = $1 WHERE id = $2`, [jurisdiction.public_url, jurisdiction.id])
        }
    }
