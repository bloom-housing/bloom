...
export class addPickUpAndDropOffColumnsToListing1624959910201 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "listings_application_address_type_enum" AS ENUM('leasingAgent', 'mailingAddress')`
    )
    await queryRunner.query(
      `ALTER TABLE "listings" ADD "application_pick_up_address_type" "listings_application_address_type_enum"`
    )
    await queryRunner.query(
      `ALTER TABLE "listings" ADD "application_drop_off_address_type" "listings_application_address_type_enum"`
    )
  }
