...
export class convertListingAddressesJsonbsToTables1626258763008 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const listings = await queryRunner.query(
      `SELECT id, application_address, leasing_agent_address, application_pick_up_address, application_mailing_address, application_drop_off_address FROM listings`
    )
    for (const listing of listings) {
      const addressKeys = [
        "application_address",
        "leasing_agent_address",
        "application_pick_up_address",
        "application_mailing_address",
        "application_drop_off_address",
      ]
