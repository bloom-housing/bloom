...
export class addJurisdictionsTable1624272587523 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "jurisdictions" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "name" text NOT NULL, CONSTRAINT "PK_7cc0bed21c9e2b32866c1109ec5" PRIMARY KEY ("id"))`
    )
    ...
    for (const jurisdictionName of [CountyCode.detroit, CountyCode.alameda]) {
      const jurisdiction = await queryRunner.query(
        `INSERT INTO "jurisdictions" (name) VALUES ($1)`,
        [jurisdictionName]
      )
      const listingsMatchingJurisdiction = await queryRunner.query(
        `SELECT id from listings where county_code = ($1)`,
        [jurisdictionName]
      )
      for (const listing of listingsMatchingJurisdiction) {
        await queryRunner.query(`UPDATE listings SET jurisdiction_id = ($1) WHERE id = ($2)`, [
          jurisdiction.id,
          listing.id,
        ])
      }
