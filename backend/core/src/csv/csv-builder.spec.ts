...
describe("CSVBuilder", () => {
  ...
  it("create empty response", () => {
    const response = service.build([], () => [], false)
    expect(response).toBe("")
  })
  it("create only headers for empty data array", () => {
    const response = service.build([], testMetadataAggregateFactory, true)
    expect(response).toBe('"Foo","Bar"\n')
  })
  it("should not include headers when such option is specified", () => {
    const response = service.build([], testMetadataAggregateFactory, false)
    expect(response).toBe("")
  })
  it("create correctly escaped CSV for correct data", () => {
    const response = service.build([{ foo: "foo", bar: "bar" }], testMetadataAggregateFactory, true)
    expect(response).toBe('"Foo","Bar"\n"foo","bar"\n')
  })
  it("create CSV with escaped double quotes", () => {
    const response = service.build([{ foo: '"', bar: "bar" }], testMetadataAggregateFactory, true)
    expect(response).toBe('"Foo","Bar"\n"""","bar"\n')
  })
  ...
  it("create an empty CSV for array type of metadata for undefined input array", () => {
    const response = service.build([], testMetadataAggregateWithArrayFactory, true)
    expect(response).toBe("")
  })
  it("create an empty CSV for array type of metadata for null input array", () => {
    const response = service.build([{ arr: null }], testMetadataAggregateWithArrayFactory, true)
    expect(response).toBe("")
  })
  it("create an empty CSV for array type of metadata for empty input array", () => {
    const response = service.build([{ arr: [] }], testMetadataAggregateWithArrayFactory, true)
    expect(response).toBe("")
  })
