...
export class OrderQueryParamValidator implements ValidatorConstraintInterface {
  validate(order: Array<string> | undefined, args: ValidationArguments) {
    if (args.property === 'order') {
      return Array.isArray(order) ? (args.object as ListingsQueryParams).orderBy?.length === order.length : false
    } else if (args.property === 'orderBy') {
      return Array.isArray(order) ? (args.object as ListingsQueryParams).order?.length === order.length : false
    }
    return false
  }
